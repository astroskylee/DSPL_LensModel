...
from svi_utils import SVI_vec, plot_loss, split_scheduler
...
def reduce_bg(data, corner_pixel=5):
    background_pixels = np.vstack([
        data[:corner_pixel, :corner_pixel],
        data[:corner_pixel, -corner_pixel:],
        data[-corner_pixel:, :corner_pixel],
        data[-corner_pixel:, -corner_pixel:],
    ])
...
def get_pixel_grid(data, pix_scale, ss=1):
    return Geometry.get_pixel_grid(data, pix_scale, ss=ss)


def reduced_distance_matrix(a):
    return Geometry.reduced_distance_matrix(a)


def get_value_from_index(xs, i):
    return ResumeInit.get_value_from_index(xs, i)


def init_to_value_or_defer(site=None, values=None, defer=numpyro.infer.init_to_median):
    return ResumeInit.init_to_value_or_defer(site=site, values=values, defer=defer)
...
def EPL_w_shear(
    ...
    with numpyro.plate(f'{plate_name} scalers - [1]', 1):
        ...
        with numpyro.plate(f'{plate_name} vectors - [2]', 2):
            e_mass = numpyro.sample(
                f'e_{param_name}',
                dist.TruncatedNormal(0.0, 0.25, low=e_low, high=e_high),
            )
    ...
    if center_x is None:
        center = numpyro.sample(
            ...
            dist.TruncatedNormal(0.0, 0.1, low=center_low, high=center_high).expand([2]),
        )
    else:
        center = numpyro.deterministic(
            f'center_{param_name}',
            jnp.array([center_x, center_y], dtype=jnp.float64),
        )
    return [{
        'theta_E': theta_e[0],
        'gamma': gamma[0],
...
def params2kwargs_multi_gauss_light(params, param_name, sigma_lims=None):
    return [{
...
def SIE(plate_name, param_name, origin, theta_low=0.0, theta_high=0.01):
    with numpyro.plate(f'{plate_name} scalers - [1]', 1):
        theta_e = numpyro.sample(f'theta_E_{param_name}', dist.Uniform(theta_low, theta_high))
        center_0 = numpyro.deterministic(f'center_1_{param_name}', jnp.array([origin[0]], dtype=jnp.float64))
        center_1 = numpyro.deterministic(f'center_2_{param_name}', jnp.array([origin[1]], dtype=jnp.float64))
        with numpyro.plate(f'{plate_name} vectors - [2]', 2):
            e_mass = numpyro.sample(
                f'e_{param_name}',
                dist.TruncatedNormal(0.0, 0.25, low=-0.4, high=0.4),
            )
    return [{
        'theta_E': theta_e[0],
        'center_x': center_0[0],
...
def matern_power_spectrum(*args, **kwargs):
    return PowerSpectrum.matern_power_spectrum(*args, **kwargs)


def params2kwargs_power_spectrum(params, param_name):
    return PowerSpectrum.params2kwargs(params, param_name)
...
def fit_power_spectrum(
    ...
    svi_map = image_guide.median(svi_result.params)
    return svi_result, svi_map


@partial(jax.vmap, in_axes=(0, 0, None, None))
def source_power_spectrum(image, rng_key_, n_value, positive=True):
...
def import_function(namespace=None):
    ...
    namespace.update({
        ...
        'Geometry': Geometry,
        'reduce_bg': reduce_bg,
        'get_pixel_grid': get_pixel_grid,
        'reduced_distance_matrix': reduced_distance_matrix,
        'get_value_from_index': get_value_from_index,
        'median_params2kwargs': median_params2kwargs,
        'init_to_value_or_defer': init_to_value_or_defer,
        'PowerSpectrum': PowerSpectrum,
        'Mass': Mass,
        'EPL_w_shear': EPL_w_shear,
        'params2kwargs_EPL_w_shear': params2kwargs_EPL_w_shear,
        'multi_gauss_light': multi_gauss_light,
        'params2kwargs_multi_gauss_light': params2kwargs_multi_gauss_light,
        'SIE': SIE,
        'params2kwargs_SIE': params2kwargs_SIE,
        'matern_power_spectrum': matern_power_spectrum,
        'params2kwargs_power_spectrum': params2kwargs_power_spectrum,
        'power_spectrum_model': power_spectrum_model,
        'fit_power_spectrum': fit_power_spectrum,
        'source_power_spectrum': source_power_spectrum,
        'pixelize_plane_multiplane': pixelize_plane_multiplane,
...
def import_function_DSPL_legacy(namespace=None):
    if namespace is None:
        namespace = inspect.currentframe().f_back.f_globals
    ...
    namespace.update({
        ...
        'MPLightModel': MPLightModel,
        'MPMassModel': MPMassModel,
        'pixelize_plane': pixelize_plane_multiplane,
    })
    return namespace
